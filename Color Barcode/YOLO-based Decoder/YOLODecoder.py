...
def estimating(r, g, b, channel_matrix):
...
def GetGraph(img, y_start, y_end, pos):
    blue, green, red = cv2.split(img)
    b_list = []
    g_list = []
    r_list = []
...
if __name__ == '__main__':
    video_path = "90_3bit_res/90_3bit.mp4"
    txt_path = "90_3bit_res/box_coord.txt"
    ...
    while True:
        ...
        if ret == True:
            ...
            # Differential 미적용
            #and_image = boxed_image
            # y_left_top = box_y
            # y_right_bottom = box_y+box_width

            if flag_sync==True:
                print("Receiving Data Frames")

                r, g, b = GetColor(and_image, y_left_top, y_right_bottom, pos)
                #GetGraph(and_image, y_left_top, y_right_bottom, pos)
                ...
                # Barcode 외에 다른 Labelling이 검출된 경우
                if len(frames_list) > 1:
                    if prev_frame == -1:
                        if frames_list[1] == 1:
                            prev_frame = frames_list[1]
                            print("Start - Synchronizing: Type 1", prev_frame)
                            frame_type = 1
                        elif frames_list[1] == 2:
                            prev_frame = frames_list[1]
                            print("Start - Synchronizing: Type 2", prev_frame)
                            frame_type = 2
                        else:
                            print("Missing Frame")
                        ...
                        if prev_frame == 0:
                            ...
                            if prev_frame % 2 == 1:
                                if prev_frame + 2 == frames_list[1]:
                                    prev_frame += 2
                                    print("Try- Synchronizing : Type 1", prev_frame, frames_list[1])
                                else:
                                    prev_frame = frames_list[1]
                                    print("Failed - Synchronizing : Type 1", prev_frame, frames_list[1])
                            else:
                                if prev_frame + 2 == frames_list[1]:
                                    prev_frame += 2
                                    print("Try- Synchronizing : Type 2", prev_frame, frames_list[1])
                                else:
                                    prev_frame = frames_list[1]
                                    print("Failed - Synchronizing : Type 2", prev_frame, frames_list[1])

                    # plt.plot(boxed_image)
                    # plt.tight_layout()
                    # plt.show()

                    cv2.imshow(winName,and_image)
                    cv2.imwrite('barcode.png', and_image)
                    rgb_emax, rgb_emin, rgb_max, rgb_min = GetPixelGraph(and_image, y_left_top, y_right_bottom, frame_type)
                    ...
                    if frame_type == 1:
                        if frames_list[1] == 1:
                            ...
                            GetGraph(and_image, y_left_top, y_right_bottom, pos_cand)

                        elif frames_list[1] == 3:
                            ...
                            if len(g_emax):
                                g_min = int((np.array(g_emax).mean() + g_max)/2)
                            if len(b_emin):
                                b_min = int((np.array(b_emin).mean() + b_min)/2)
                            ...
                            if len(g_emin):
                                g_min = int((np.array(g_emin).mean() + g_min)/2)
                            if len(b_emax):
                                b_min = int((np.array(b_emax).mean() + b_max)/2)
                    ...
                    # Synchronized
                    if prev_frame == 5 or prev_frame == 6:
                        print("Synchronized!!")
                        flag_sync=True
