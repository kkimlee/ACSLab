...
# pilot_range를 지정해서
# pilot frame을 추적
pilot_range = np.array([[180, 50, 60], [40,125,120], [30, 190, 40], [120, 70, 145], [55, 40, 190], [135, 130, 65]])
...
def findRCluster():
    ...
    if end_idx == -1 and start_idx >= 0:
        end_idx = bar_length - 1
...
#################################
# Path, File List
path = "./frames_anything1/"
file_list = os.listdir(path)
f = open("95_3bit_anal.txt", 'w')
...
pilot_range[:, 1] = pilot_range[:, 1]# + 40
pilot_range[:, 2] = pilot_range[:, 2]# + 40

for file in file_list[:]:
    img = cv2.imread(path+file)
    # crop
    img_crop = img[38:980, 1070:1085, :]
    ...
    g_mean = rotated_g.mean(axis=1)[:].T# + 40
    b_mean = rotated_b.mean(axis=1)[:].T# + 40
    ...
    # plt.tight_layout()
    # if idx >= 100:
    #     plt.savefig('./figures_anything/' + str(idx) + '.png', dpi=300)
    # elif idx >= 10:
    #     plt.savefig('./figures_anything/0' + str(idx) + '.png', dpi=300)
    # else:
    #     plt.savefig('./figures_anything/00' + str(idx) + '.png', dpi=300)
    # plt.close()
    idx += 1
    ...
    if flag == 0 and last_pilot < 7:
        ...
        # print(last_pilot, pilot_list[0])
        # if last_pilot != pilot_list[0] and last_pilot + 1 != pilot_list[0]:
        ...
        # flg_cont = 0
        # for p_idx in range(1, len(pilot_list)):
        #     if pilot_list[p_idx - 1] != pilot_list[p_idx] - 1:
        #         flg_cont = 1
        #         break
        #
        # if flg_cont == 1:
        ...
        if last_pilot == 7:
            ...
            print(p1, p2, p3)
            data_pos = [p1%bar_length, p2%bar_length, p3%bar_length]

            # if data_pos >= bar_length:
            #     data_pos = data_pos % bar_length
            ...
            if pos_pilot[len(pos_pilot) - 1] < data_pos[data_cnt%3] and data_pos[data_cnt%3] < bar_length:  # pilot frame 마지막과 data frame이 같이 있는 경우
                data_cnt += 1

                vxline.append([data_pos[data_cnt], 7])
                print("Data Pos: ", data_pos[data_cnt%3])
                rx_color = estimating(data_pos[data_cnt%3])
                rx_data = decoding(rx_color)
                rx_data_list.append(rx_data)

            print(data_pos[data_cnt%3])

            frame_gap = getFrameGap(pos_pilot)
    else:
        data_cnt += 1

        vxline.append([data_pos[data_cnt%3], 7])
        print(data_pos[data_cnt%3])
        rx_color = estimating(data_pos[data_cnt%3])
        rx_data = decoding(rx_color)
        rx_data_list.append(rx_data)
        ...
        if data_cnt >= 27:
            ...
            # Data Frame 마지막 + Pilot의 RR`인 경우 처리
            start, end = findRCluster()
            mid = getMid(start, end)
            print("Red: ", start, end, mid)
            if mid != -1 and last_pilot <= 1:
